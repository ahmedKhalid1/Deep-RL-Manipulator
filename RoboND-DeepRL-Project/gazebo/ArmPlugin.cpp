 ...
 * Modified again by - Ahmed Khalid
 */
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		bool isGripperLink = !(strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT));
		bool isTube = !(strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM));
		if (isGripperLink && isTube) // For the second objective
		//if (isTube) // For the first objective
		{
			rewardHistory = REWARD_WIN;

			newReward  = true;
			endEpisode = true;
			// return at first sight of the wanted collision to avoid being over-written by another unwanted collision
			return;
		}
		else // For any other unwanted collision
		{
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	float velocity = vel[action/2] + actionVelDelta * ((action  % 2 == 0)? 1.0 : -1.0);; // TODO - Set joint velocity based on whether action is even or odd.
	...
	/
	*/
	float joint = ref[action/2] + actionJointDelta * ((action  % 2 == 0)? 1.0 : -1.0); // TODO - Set joint position based on whether action is even or odd.
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/
		...
		bool hitGround = gripBBox.max.z <= groundContact || gripBBox.min.z <= groundContact;
		if(hitGround)
		{
		...
		*/ 
		...
		if(!hitGround)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				// Working well for position control
				const float distDelta  = lastGoalDistance - distGoal;
				const float changeRate = 0.9f;
				const float penalty = 0.5f; // -ive reward for not reaching the goal yet	 
				// compute the smoothed moving average of the delta of the distance to the goal
				avgGoalDelta  = (avgGoalDelta * changeRate) + (distDelta * (1.0f - changeRate));
				rewardHistory = 10 * avgGoalDelta - penalty;
				
				/*
				const float distDelta  = lastGoalDistance - distGoal;
				const float changeRate = 0.9f;
				const float penalty = 1.5f; // -ive reward for not reaching the goal yet	 
				// compute the smoothed moving average of the delta of the distance to the goal
				avgGoalDelta  = (avgGoalDelta * changeRate) + (distDelta * (1.0f - changeRate));
				rewardHistory = 10 * avgGoalDelta - penalty;
				*/
				//rewardHistory = avgGoalDelta;
				//printf("interim reward is '%f'\n", rewardHistory);
				newReward = true;	
			}
